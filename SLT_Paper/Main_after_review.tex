...
Evaluation, however, often relies on text-based measures, task success, or subjective ratings, which provide limited direct measures of speech-native interactional behavior. 
Systems trained primarily on read speech may not represent properties characteristic of spontaneous dialogue \cite{omahony_combining_2022}.
...
First, we construct matched reference regimes for conversational $F_0$ mean, $F_0$ expressivity, speech rate, articulation rate, pause ratio, and mean pause duration\footnote{The code will be made publicly available upon acceptance.}. 
...
\textcolor{green}{The novelty is not the individual acoustic measures, but their use in a calibrated matched-reference evaluation protocol.}
...
In particular, $F_0$ SD and $F_0$ range increase monotonically across arousal and dominance sextiles obtained with Vox-Profile. 
...
Model-predicted age-bin effects are much smaller than the main sex-label and state-conditioned shifts, so the practical implication is that we treat age-conditioned references as optional and exploratory, to be used only when age estimates are reliable and appropriate for the evaluation setting.
...
\textcolor{green}{This protocol is intended as a behavioral plausibility check, not as a complete naturalness score. 
It does not evaluate semantic appropriateness, task success, user satisfaction, or dialogue policy. Instead, it identifies whether specific prosodic and rhythmic measurements are atypical relative to a stated human reference stratum.}
...
\begin{enumerate}
    ...
    \item Flag metrics below the 5th percentile or above the 95th percentile as out-of-regime, and report the output as a vector of percentile deviations rather than a single opaque score. \textcolor{green}{Since some temporal measurements are algebraically related, multiple temporal flags should be interpreted jointly as a decomposition of timing behavior rather than as independent evidence of separate failures.}
\end{enumerate}

\begin{table}[t]
\centering
\caption{Recommended use of matched references.}
...
Cue & Recommended use \\
\midrule
Mean $F_0$ & Match by model-predicted sex label when appropriate; otherwise report pooled. \\
$F_0$ SD/range & Match by arousal/dominance only when state estimates are reliable; if output-derived, also report coarser. \\
Rate/pausing & Interpret rate, articulation rate, and pause ratio jointly, not as independent flags. \\
Age bin & Optional/exploratory; use only when reliable and appropriate. \\
\bottomrule
\end{tabular}
\end{table}

\textcolor{green}{Table~\ref{tab:practical_guidance} summarizes recommended matching choices, emphasizing that conditioning should be used only when the corresponding variable is reliable and appropriate for the evaluation setting.}
...
\section{Human-Rated Sanity Check}\label{sec:human_eval_check}

As a small external check, we applied the protocol to human-rated dyads from TRACE \cite{TRACE}, which includes original Seamless conversations and EmotiVoice resyntheses designed to disrupt conversational entrainment. 
Ten native English-speaking listeners rated full two-speaker clips on 1--5 naturalness and rhythm/timing scales. 
We aggregate ratings at the clip level and score only clips that match the conditions listed in section \ref{sec:methods}: 14 Original clips and 13 EmotiVoice TTS clips.
For Original clips, we score all matched role-level segments, and for EmotiVoice TTS, we score only role-level segments marked as synthesized.

\begin{table}[t]
\centering
\caption{Human-rated sanity check on exact feature-matched clips. Out-of-regime (OOR) is the mean percentage of role-level metric comparisons outside matched 5th--95th percentile Seamless references; percentile distance ($P_{dist}$) is the mean absolute distance from the matched reference median percentile.}
\label{tab:human_eval_reference_check}
\small
\setlength{\tabcolsep}{3pt}
\begin{tabular}{l c c c c c}
\toprule
\textbf{Condition} & \textbf{Clips} & \textbf{Naturalness} & \textbf{Rhythm} & \textbf{OOR} & \textbf{$P_{dist}$} \\
\midrule
Original & 14 & $4.79 \pm 0.24$ & $4.74 \pm 0.43$ & 13.4\% & 26.9 \\
EmotiVoice TTS & 13 & $2.29 \pm 0.80$ & $2.44 \pm 0.54$ & 24.2\% & 29.5 \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:human_eval_reference_check} shows that the lower-rated EmotiVoice TTS clips also produce more matched-reference out-of-regime flags. 
The effect is more concentrated in specific cues: synthesized segments are most often atypical for mean $F_0$ (69.0\% for EmotiVoice TTS vs. 20.2\% for Original) and pause ratio (22.9\% vs. 7.1\%). 
This supports the intended use of the protocol as an interpretable cue-level plausibility check that complements human evaluation.
...
\textcolor{green}{State-conditioned references may be circular when the state estimate is inferred from the same generated waveform being scored, but this can be mitigated by reporting coarser references or using external/context-derived state estimates.}
...
Future work should validate these regimes against larger human-judgment studies and system outputs across multiple datasets and languages.
The small human-rated check in Section~\ref{sec:human_eval_check} is encouraging but does not establish perceptual thresholds, nor does it map percentile deviations directly to user-rated naturalness or interaction quality. 
Moreover, the audio quality was also slightly lower for EmotiVoice TTS (3.71 vs. 4.95), so this check does not completely isolate prosody and rhythm from synthesis artifacts. 
...
The authors used Claude (Anthropic) to assist with grammar checking and proofreading the manuscript. These tools were not used to generate any scientific content, results, or conclusions. All authors reviewed and take full responsibility for the final content of this paper.
