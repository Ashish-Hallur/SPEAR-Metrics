...
We aggregate ratings at the clip level and score only clips with exact acoustic-feature matches: 14 Original clips and 13 EmotiVoice TTS clips.
...
\begin{table}[t]
\centering
\caption{Human-rated sanity check on exact feature-matched clips. Out-of-regime is the mean percentage of role-level metric comparisons outside matched 5th--95th percentile Seamless references; percentile distance is the mean absolute distance from the matched reference median percentile.}
\label{tab:human_eval_reference_check}
\small
\setlength{\tabcolsep}{3pt}
\begin{tabular}{l r c c r r}
\toprule
\textbf{Condition} & \textbf{Clips} & \textbf{Naturalness} & \textbf{Rhythm} & \textbf{Out \%} & \textbf{Pct. dist.} \\
\midrule
